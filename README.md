...
- [x] Module Agenda — vue calendrier visuelle façon Google Calendar avec trois modes **Jour / Semaine / Mois** (navigation ‹ Aujourd'hui ›, grille horaire avec les événements positionnés selon leur heure/durée, ligne rouge indiquant l'heure actuelle, ligne "jour" pour les éléments sans horaire) : événements locaux (ajout/modification/suppression, écriture directe dans `monhub-data`), lecture seule de Google Calendar en complément (voir configuration ci-dessous), et affichage des échéances des autres modules (tâches, échéances voiture, expirations de documents, rendez-vous et renouvellements santé, échéances d'objectifs) — clique sur un événement pour le modifier, sur un élément d'un autre module pour y aller directement, ou sur un jour en vue Mois pour zoomer dessus. Une échéance dont la date est dépassée et qui n'est pas encore marquée faite n'est jamais masquée : elle apparaît dans une section « En retard » en haut de l'Agenda plutôt que de disparaître silencieusement. Une échéance voiture uniquement au kilométrage (ex: vidange à 65 000 km, sans date) n'a pas de jour à placer sur le calendrier tant qu'elle n'est pas due, mais dès que le kilométrage actuel du véhicule la dépasse, elle apparaît elle aussi dans « En retard ».
- [x] Module Tâches — todo-list avec catégories, priorités, échéances (avec détection de retard), section "Terminées" repliable, et **récurrence** optionnelle (chaque jour/semaine/2 semaines/mois/3 mois/6 mois/année) : une tâche récurrente marquée faite n'est jamais close définitivement, elle est directement reprogrammée à la prochaine échéance
- [x] Module Habitudes — suivi **quotidien**, **hebdomadaire** ou **tous les N jours** (fréquence libre, ex: tous les 2, 3, 4 jours…) avec streaks et mini heatmap des 14 derniers jours. **Détection des séries rompues** : dès qu'une série d'au moins 2 périodes vient de se rompre (période manquée après une série active), un avertissement apparaît directement sur la carte de l'habitude, dans les rappels de l'Aperçu et sur la pastille de navigation — pour être prévenu activement plutôt que de le remarquer par hasard
- [x] Module Voiture — véhicule(s), échéances (contrôle technique, vidange, assurance) avec alerte de retard et statut fait/à faire, journal d'entretien avec coût total (calculé sur les entretiens faits uniquement) et statut fait/prévu — les échéances et entretiens non faits apparaissent dans les rappels et sur l'Agenda ; une fois marqués faits, ils en disparaissent. Une échéance peut être fixée par **date**, par **kilométrage** (ex: vidange à 65 000 km), ou les deux à la fois — au moins l'un des deux est requis. Une échéance au kilométrage se compare au kilométrage actuel du véhicule et n'a pas de date propre, donc elle n'apparaît pas sur l'Agenda (qui reste un calendrier par date) mais reste visible dans le module Voiture et dans les rappels de l'Aperçu. Une échéance peut aussi être **récurrente** (tous les X mois et/ou tous les Y km) : marquée faite, elle se reprogramme automatiquement à la prochaine occurrence au lieu d'être close, et un entretien correspondant est ajouté au journal pour garder une trace. En créant une échéance, la récurrence est **suggérée automatiquement** si l'historique d'entretien connaît déjà ce type (moyenne des écarts entre les enregistrements passés du même nom pour ce véhicule) — libre à toi de l'ajuster ou de l'effacer. Chaque échéance peut être dupliquée en un clic (⧉)
- [x] Module Documents — références importantes par catégorie avec dates d'expiration (alerte expiré/bientôt expiré) et statut renouvelé/à renouveler — une fois marqué renouvelé, un document n'apparaît plus dans les rappels ni sur l'Agenda — pas de liens Drive (non souhaité). Un document peut avoir un **renouvellement automatique** (tous les X mois, avec des raccourcis "tous les ans/2 ans/5 ans/10 ans") : marqué renouvelé, il avance directement à la prochaine échéance au lieu d'être clos. L'intervalle est **suggéré automatiquement** selon le nom du document (passeport/CNI → 10 ans, permis de conduire → 15 ans, assurance/mutuelle → 1 an, contrôle technique → 2 ans), ajustable ou effaçable. Chaque document peut être dupliqué en un clic (⧉)
...
- [x] Module Voyages — voyages (destination, dates, budget), dépenses par voyage avec suivi budget vs réalisé, voyages passés grisés. **Alerte croisée avec Documents** : si un document d'identité (passeport, carte d'identité, visa) sera déjà expiré avant la fin d'un voyage à venir, un avertissement apparaît directement sous le voyage concerné et dans les rappels de l'Aperçu — un rapprochement que les deux modules ne feraient pas d'eux-mêmes
...
- [x] Fonctionnalités intelligentes (IA Gemini, optionnel) — décomposition de tâches en étapes, aide pour démarrer une tâche bloquée, suggestions de sites/outils utiles (tâches, objectifs, voyages), génération de plan d'action pour les objectifs, suggestions de lieux à visiter pour les voyages, conseils pour tenir ses habitudes, analyse mensuelle des finances, plan de la journée priorisé, ajout rapide en langage naturel et **assistant conversationnel unique** sur l'onglet Aperçu
- [x] Recherche globale — barre de recherche dans l'en-tête qui retrouve instantanément une tâche, un événement, une habitude, un véhicule/échéance/entretien, un document, un rendez-vous/traitement santé, un objectif, un voyage, une note (titre, tags et contenu) ou un contact, et navigue directement vers le bon module
- [x] Module Notes — second cerveau façon mind map : notes pro et perso séparées (filtre Tous/Perso/Pro), vue **Liste** classique (recherche locale, épingler, tags) et vue **Constellation** (graphe visuel façon carte mentale). Les notes se relient entre elles automatiquement selon leur pertinence, sans aucune IA nécessaire : lien explicite en écrivant `[[Titre d'une autre note]]` dans le contenu, tag partagé entre deux notes, ou vocabulaire proche (mots-clés significatifs en commun) détecté par similarité de Jaccard — trois types de lien affichés avec des couleurs différentes dans la constellation. Cliquer sur une note dans la constellation ouvre un panneau de détail avec ses notes liées (cliquables pour naviguer de proche en proche). Avec l'IA activée, un bouton « ✨ Suggérer des notes liées » propose en plus des rapprochements plus subtils qu'un simple mot-clé, à valider un par un (jamais de liaison automatique)
- [x] **Rappel de résurgence sur une note** : une note peut porter une date "me rappeler cette note le…" — ce jour-là, elle apparaît automatiquement dans les rappels de l'Aperçu et sur l'Agenda, exactement comme une tâche ou un rendez-vous, pour resurgir au bon moment plutôt que de rester invisible tant qu'on ne la cherche pas activement
- [x] **Notes ↔ autres modules** : depuis le panneau de détail d'une note (vue Constellation), on peut la rattacher explicitement à un élément de n'importe quel autre module (tâche, événement, habitude, véhicule/échéance/entretien, document, rendez-vous/traitement santé, objectif, voyage, contact) via une recherche instantanée — affiché en chip cliquable des deux côtés. Chaque élément lié affiche en retour un petit badge « 📝 N note(s) » qui ramène directement vers l'onglet Notes
- [x] Module Contacts — personnes importantes (relation, anniversaire, idées cadeaux, notes), triées par prochain anniversaire avec âge calculé automatiquement
- [x] **Rappels intelligents avec conseil actionnable** : en plus de la date, certains rappels affichent un petit conseil déduit du type d'échéance (aucune IA, juste des mots-clés) — un anniversaire dans les 7 jours suggère « 🎁 Pense à un cadeau » (partout où l'idée cadeau du contact est visible si renseignée), un contrôle technique suggère « 📅 Pense à prendre rendez-vous chez le contrôleur », une vidange « 🔧 Pense à réserver un créneau garage », un passeport/une carte d'identité qui expire « 🪪 Pense à prendre rendez-vous en mairie (délais possibles) », une assurance/mutuelle qui se renouvelle « 💶 Compare les offres avant de renouveler »
- [x] **Bilan hebdomadaire** dans l'Aperçu : un résumé compact de "cette semaine" (tâches à traiter, habitudes du jour, échéances à suivre, variation du patrimoine depuis le dernier relevé) et la liste des anniversaires à venir sous 7 jours avec leur conseil cadeau — recalculé à chaque ouverture à partir des données déjà existantes, sans stockage supplémentaire
...
- **Assistant conversationnel unique** : une zone de question libre sur l'Aperçu (« ✨ Demande-moi n'importe quoi sur tes données ») permet de poser n'importe quelle question sur ses propres données (ex : *"Combien j'ai dépensé en voiture ce trimestre ?"*, *"Quand expire mon passeport ?"*). L'IA reçoit un résumé de toutes les données (tâches, agenda, habitudes, voiture, documents, santé, objectifs, voyages, finances, notes) et répond uniquement à partir de ce résumé — elle dit explicitement quand une information demandée n'y figure pas plutôt que de l'inventer. Question posée en lecture seule : l'assistant ne modifie jamais de donnée.
