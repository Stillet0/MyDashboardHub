...
export default function FinancesUpdate({ data, saving, onSave }: Props) {
  ...
  async function handleSave() {
    ...
    const newCashflow: Cashflow = {
      ...
      income,
      expenses,
      movements: existingCf?.movements ?? {},
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-[20px] border border-[var(--border)] bg-[var(--surface)] p-6">
        <div className="mb-3 text-sm font-medium text-[var(--text-muted)]">Comptes — valeurs</div>
        <div className="divide-y divide-[var(--border)]">
          {data.accounts.map((acc) => (
            <div key={acc.id} className="flex items-center justify-between gap-3 py-3">
              <div>
